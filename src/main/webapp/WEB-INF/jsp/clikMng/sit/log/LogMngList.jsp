...
function fnExcel() {
	var varForm = document.listForm;
	varForm.action = "<c:url value='/sit/log/LogMngExcel.do'/>";
	varForm.submit();
}
...
<DIV id="page-wrapper">
	<div class="row">
        ...
        <!-- /.col-lg-12 --> 
    			...
    			<DIV class="panel-heading">접속로그 목록 및 상세정보</DIV>
			...
			<ul class="nav nav-tabs">
				...
				<form:form id="listForm" name="listForm" action="" method="post">
					...
					<div class="select_box">
						<span>
							<select id ="selSearchGubun" name="selSearchGubun" style="width:150px;">
					    		<option value="selId" <c:if test="${searchVO.selSearchGubun == 'selId'}">selected</c:if>>아이디</option>
					    		<option value="selNm" <c:if test="${searchVO.selSearchGubun == 'selNm'}">selected</c:if>>이름</option>
					    		<%-- <option value="selTy" <c:if test="${searchVO.selSearchGubun == 'selTy'}">selected</c:if>>유형</option> --%>
					    	</select>

					    	<input name="searchKeyword" type="text" value='<c:out value="${searchVO.searchKeyword}"/>' title="검색어 입력"  >
							<a href="#none"><button type="button" class="btn btn-primary" onclick="fnSearch();"><spring:message code="button.search" /></button></a> 
					...
					<table width="100%" cellpadding="8" class="table table-striped table-bordered" summary="로그 관리 테이블입니다. 번호, 아이디, 이름, 접속일시, 로그유형, 접속IP 등 정보를 담고 있습니다.">
						<thead>
						<tr>
							<th class="title" width="10%" scope="col" nowrap="nowrap">번호</th>
							<th class="title" width="30%" scope="col" nowrap="nowrap">아이디</th>
							<th class="title" width="15%" scope="col" nowrap="nowrap">이름</th>
							<th class="title" width="10%" scope="col" nowrap="nowrap">접속일시</th>
							<th class="title" width="10%" scope="col" nowrap="nowrap">로그유형</th>
							<th class="title" width="15%" scope="col" nowrap="nowrap">접속IP</th>
						</tr>
					 	</thead>
						...
						<%-- 데이터를 없을때 화면에 메세지를 출력해준다 끝 --%>
					 	...
					 	<c:forEach var="x" items="${resultList}" varStatus="status">
						 	<tr>
							    ...
							    <td class="lt_text3" nowrap="nowrap"><c:out value="${x.conectId}" /></td>
							    <td class="lt_text3" nowrap="nowrap"><c:out value="${x.conectNm}" /></td>
							    <td class="lt_text3" nowrap="nowrap"><c:out value="${x.creatDt}" /></td>
							    <td class="lt_text3" nowrap="nowrap"><c:out value="${x.conectMthd}" /></td>
							    <td class="lt_text3" nowrap="nowrap"><c:out value="${x.conectIp}" /></td>
						 	</tr>
					 	</c:forEach>
					 	</tbody>
					</table>
