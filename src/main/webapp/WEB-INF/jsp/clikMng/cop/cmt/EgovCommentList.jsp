...
<h2>댓글 - <b class="r"><c:out value="${resultCnt}"/></b>개</h2>

 
<table width="100%" cellpadding="8" class="table table-striped table-bordered table-hover">
	<tbody>
			...
			<tr>
				<td class="listLeft" width="70%" style="padding-left:10px;" nowrap>
		    		<c:choose>
		    			...
		    			<b class="b"><c:out value="${result.wrterNm}" /></b>&nbsp;
		    			</c:when>
		    			<c:otherwise>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" class="table table-striped table-bordered table-hover">
	<tr>
		<th width="20%" height="23"  nowrap>작성자
	...
	<div align="center">
		  ...
		  <c:choose>
			<c:when test="${searchVO.commentNo == ''}">
			   <a href="#none" onclick="javascript:fn_egov_insert_commentList()"><button type="button" class="btn btn-primary">댓글등록</button></a>
			</c:when>
			<c:otherwise>
			  <a href="#none" onclick="javascript:fn_egov_updt_commentList()"><button type="button" class="btn btn-success">댓글수정</button></a>
			</c:otherwise>
		  </c:choose>
		  <a href="#none" onclick="javascript:fn_egov_select_commentList('1')"><button type="button" class="btn btn-danger">댓글초기화</button></a>
	</div>
