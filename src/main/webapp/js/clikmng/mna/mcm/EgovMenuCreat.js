...
function addTreeNode(parentNode, recursedNodes) {
	for (var i = 0; i < treeNodes.length; i++) {
		...
		if (nodeValues[1] == parentNode) {
			...
			document.write("onclick='javascript:fCheckDir(this, this.name, this.value,"+i+");' value=" + nodeValues[0] + ">");
			if (hasChildNode) {
 ...
 * 모두선택 처리 함수
 ******************************************************** */
function fCheckDir(obj, fCheckYB, fValue, fPath){
	
	var checkField = document.getElementsByName(fCheckYB);

	var target = $('#div' + $(obj).val());
	
	if(obj.checked == true)
	{
		$.each(target.find('input:checkbox'), function(idx, checkbox) {
