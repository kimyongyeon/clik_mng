// Node object
function Node(id, pid, name, url, title, target, icon, iconOpen, open) {	//dtree.add(id, pid, 노드이름, URL_이벤트_링크, 툴팁, target, 닫힌상태의 아이콘, 열린상태의 아이콘, open);		// 현재 메뉴 아이디
	this.id = id;	// 상위 메뉴 아이디
	this.pid = pid;	// 메뉴 명
	this.name = name;	// 메뉴 주소
	this.url = url;	// 링크 주소
...
dTree.prototype.addNode = function(pNode) {
	...
	for (n; n<this.aNodes.length; n++) {
		if (this.aNodes[n].pid == pNode.id) {
			...
			if (this.config.useSelection && cn.id == this.selectedNode && !this.selectedFound) {					//alert(this.config.useSelection +"-----"+ cn.id +"------"+ this.selectedNode +"-----"+ !this.selectedFound +"-----"+ cn.name);				
					cn._is = true;
					this.selectedNode = n;
...
dTree.prototype.node = function(node, nodeId) {		//showObj(node, nodeId);		var str = '<li>' + this.indent(node, nodeId);
	if (this.config.useIcons) {
		...
		str += '';
	}	//alert(node.target +'--------'+ node.url + '=========' + node.name + '====this.root.id---' + this.root.id);	// address 확인	if (node.url) {		if(node.url == node.title) {			str += '<a id="s' + this.obj + nodeId + '"class="on" href="' + node.url + '"';		} else {			str += '<a id="s' + this.obj + nodeId + '" href="' + node.url + '"';		}				//if (node.title) str += ' title="' + node.title + '"';
		//if (node.target) str += ' target="' + node.target + '"';
		if (this.config.useStatusText) str += ' onmouseover="window.status=\'' + node.name + '\';return true;" onmouseout="window.status=\'\';return true;" ';
		if (this.config.useSelection && ((node._hc && this.config.folderLinks) || !node._hc))
			str += ' onclick="javascript: ' + this.obj + '.s(' + nodeId + ');"';		var mark ;		if(node.target == 1){			mark = "Ⅰ.";		} else if(node.target == 2) {			mark = "Ⅱ.";		} else {			mark = "Ⅲ.";		}
		str += '>' +  mark;		//<i class="fa fa-th-list fa-fw"></i>
	}	else if ((!this.config.folderLinks || !node.url) && node._hc && node.pid != this.root.id)			//alert(node.target +'--------'+ node.url + '=========' + node.name);	str += '<a href="javascript: ' + this.obj + '.o(' + nodeId + ');" class="node">';
	str += node.name;	//alert(str);
	if (node.url || ((!this.config.folderLinks || !node.url) && node._hc)) str += '</a>';	if(node.target == 2) {		if (node._hc) {						str += '<ul id="d' + this.obj + nodeId + '" class="nav nav-third-level collapse in" style="height: auto; ">';			str += this.addNode(node);			str += '</ul>';		}			} else if(node.target == 1) {		if (node._hc) {						str += '<ul id="d' + this.obj + nodeId + '" class="nav nav-second-level collapse in" style="height: auto; ">';			str += this.addNode(node);			str += '</ul>';		}			}	str += '</li>';	this.aIndent.pop();
	return str;
};
function showObj(node, nodeId) {	var str = "";	for(key in node) {		str += key+"="+node[key]+"\n";	}	alert('node = ' + str + ', nodeid = ' + nodeId);	return;}
