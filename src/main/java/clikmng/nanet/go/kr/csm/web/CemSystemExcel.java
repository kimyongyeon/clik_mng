...
public class CemSystemExcel extends AbstractExcelView{
	...
	protected void buildExcelDocument(Map<String,Object> ModelMap
		...
		CsmManageVO searchVO = (CsmManageVO)ModelMap.get("searchVO");
		List<CsmManageVO> resultList = (List<CsmManageVO>) ModelMap.get("resultList");
		
		 String excelName = "정책정보기관별수집내역";

		 // 만들 sheet의 갯수를 정한다.
		 int maxCount = (Integer)ModelMap.get("resultCnt")/50000 ;
		 ...
		 for(int cnt = 1; cnt <= maxCount; cnt++) {
			...
			worksheet.addMergedRegion(new CellRangeAddress(0, 0, 0, 2)); // cell 병합        	
			...
			cell.setCellValue("수집건수");
			cell.setCellStyle(style);
			
			for( int i = 0 ; i < resultList.size(); i++ )
			{
				CsmManageVO vo = resultList.get(i);
				...
				//수집건수
				cell = row.createCell(2);
				cell.setCellValue(vo.getCollectionCnt());
				cell.setCellStyle(style);
