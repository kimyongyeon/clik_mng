...
public class MdmMinutesExcel extends AbstractExcelView{
	...
	protected void buildExcelDocument(Map<String,Object> ModelMap
		...
		List<MdmTnsrAsmblyMintsVO> 	resultList 		= (List<MdmTnsrAsmblyMintsVO>) ModelMap.get("resultList");
		
		 String excelName = "지방의회회의록_메타데이터";
		 ...
		 for(int cnt = 1; cnt <= maxCount; cnt++) {
			...
			worksheet.addMergedRegion(new CellRangeAddress(0, 0, 0, 12)); // cell 병합        	
...
//			cell.setCellStyle(style);
			...
			{
				MdmTnsrAsmblyMintsVO vo = resultList.get(i);
				...
				//회기
				cell = row.createCell(3);
				cell.setCellValue(vo.getRASMBLY_SESN());
				cell.setCellStyle(style);
				
				//차수
				cell = row.createCell(4);
				cell.setCellValue(vo.getMINTS_ODR());
				cell.setCellStyle(style);
...
//				cell.setCellStyle(style);
				...
				//등록일
				cell = row.createCell(6);
				cell.setCellValue(CommonUtil.formatDate(vo.getMTG_DE()!=null ? vo.getMTG_DE() : "","-"));
				cell.setCellStyle(style);
				...
				//문서번호
				cell = row.createCell(11);
				cell.setCellValue(vo.getMINTS_CN());
				cell.setCellStyle(style);
				
				//서비스URL
				cell = row.createCell(12);
				cell.setCellValue("http://clik.nanet.go.kr/potal/search/searchView.do?collection=minutes&DOCID="+vo.getMINTS_CN());
				cell.setCellStyle(style);
