...
private static byte[] pbUserKey = {
		(byte)0x75,(byte)0x01,(byte)0x65,(byte)0x41,(byte)0x56,(byte)0x73,(byte)0xAA,(byte)0xF0,
		(byte)0x02,(byte)0x78,(byte)0x84,(byte)0x24,(byte)0x80,(byte)0x01,(byte)0x13,(byte)0x01
};
...
private static void SeedEncrypt(byte pbData[], int pdwRoundKey[], byte outData[])
{
	int L0[] = new int[1];
	int L1[] = new int[1];
	int R0[] = new int[1];
...
private static void SeedEncRoundKey(int pdwRoundKey[], byte pbUserKey[])
{
	int A[] = new int[1];
	int B[] = new int[1];
	int C[] = new int[1];
...
public static String getEnc(String plainString) 
{
	byte[] plainByteStream = null;
	int plainByteStreamLength;
	byte[] blockPlain = new byte[16];
	byte[] blockCipher = new byte[16];
	String cipherString = "";
	...
	try {
		plainByteStream = plainString.getBytes("euc_kr");
	} catch (UnsupportedEncodingException e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
	}
	plainByteStreamLength = plainByteStream.length;

	for (i = 0; i < plainByteStreamLength; i += 16) {
		for (j = i; j < i+16 && j < plainByteStreamLength; j++)
			blockPlain[j-i] = plainByteStream[j];
		if (j < i+16)
			for ( ; j < i+16; j++)
				blockPlain[j-i] = (byte)0;

		SeedEncrypt(blockPlain, pdwRoundKey, blockCipher);
		cipherString += new String(encoder.encode(blockCipher));
	}

	return cipherString;
}

public static String getDec(String cipherString)
{
	byte[] cipherByteStream = null;
	int cipherByteStreamLength = 0;
	byte[] blockCipher = new byte[16];
	byte[] blockPlain = new byte[16];
	byte[] plainByteStream = null;
	...
	try {
		cipherByteStream = decoder.decodeBuffer(cipherString);
		cipherByteStreamLength = cipherByteStream.length;
		plainByteStream = new byte[cipherByteStreamLength];
	}
	catch (Exception e) {
		// do nothing ...
	}

	for (i = 0; i < cipherByteStreamLength; i += 16) {
		for (j = i; j < i+16 && j < cipherByteStreamLength; j++)
			blockCipher[j-i] = cipherByteStream[j];
		if (j < i+16)
			for ( ; j < i+16; j++)
				blockCipher[j-i] = (byte)0;

		SeedDecrypt(blockCipher, pdwRoundKey, blockPlain);

		for (j = i; j < i+16 && j < cipherByteStreamLength; j++)
			plainByteStream[j] = blockPlain[j-i];
	}
	
	try {
		return new String(plainByteStream,"euc_kr").trim();
	} catch (UnsupportedEncodingException e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
		return new String(plainByteStream).trim();
	}
}
