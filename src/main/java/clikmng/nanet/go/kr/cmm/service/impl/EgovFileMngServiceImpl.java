...
public class EgovFileMngServiceImpl extends AbstractServiceImpl implements EgovFileMngService {
     ...
     */
    @SuppressWarnings("unchecked")
    public String insertSpcFileInfs(List fvoList) throws Exception {
	String atchFileId = "";
	
	if (fvoList.size() != 0) {
	    atchFileId = fileMngDAO.insertSpcFileInfs(fvoList);
	}
	if(atchFileId == ""){
		atchFileId = null;
	}
    ...
    /**
     * 이미지 파일에 대한 목록을 조회한다. 메인 목록에 하나 출력위해
     * 
     * @see egovframework.com.cmm.service.EgovFileMngService#selectImageFileList(egovframework.com.cmm.service.FileVO)
     */
    public List<FileVO> selectImageFile(FileVO vo) throws Exception {
    	return fileMngDAO.selectImageFile(vo);
    }    
}
